...
static inline unsigned int byte_swap(unsigned int val) {
...
static void spi_read_fifo_swap(int *data, int datalen) {
  ...
  for (i = 0; i < num_words; i++) {
    ...
    unsigned int raw = *(volatile int*) (SPI_REG_RXFIFO);
    data[i] = byte_swap(raw);
  }
}
...
int main()
{
  ...
  *(volatile int*) (SPI_REG_CLKDIV) = 0x4;
  ...
  for (int i = 0; i < instr_blocks; i++) { //reads 16 4KB blocks
    // cmd 0x03 standard read, 24-bit addr
    spi_setup_cmd_addr(0x03, 8, addr << 8, 24);
    spi_set_datalen(32768);
    spi_start_transaction(SPI_CMD_RD, SPI_CSN0);
    spi_read_fifo_swap(instr, 32768);
