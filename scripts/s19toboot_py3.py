...
def s19_parse(filename, s19_dict):
    with open(filename, 'r') as s19_file:
        for line in s19_file:
            ...
            if rec_field == "S0" or prefix in ["S009", "S505", "S705", "S017", "S804"]:
                continue

            try:
                length = int("0x" + line[2:4], 0)
                if rec_field == "S1":
                    addr_len = 4
                    addr = int("0x" + line[4:8], 0)
                elif rec_field == "S2":
                    addr_len = 6
                    addr = int("0x" + line[4:10], 0)
                elif rec_field == "S3":
                    addr_len = 8
                    addr = int("0x" + line[4:12], 0)
                else:
                    continue

                # Data bytes count = length - (address bytes count + 1 checksum byte)
                data_bytes_cnt = length - (addr_len // 2 + 1)
                data_start_idx = 4 + addr_len
                
                for i in range(data_bytes_cnt):
                    byte_str = line[data_start_idx + 2*i : data_start_idx + 2*(i+1)]
                    s19_dict[addr + i] = byte_str
            except Exception as e:
                # Silently ignore format errors for non-data lines
                continue
...
""" % (int(math.log(rom_size, 2)), (rom_size - 1)))
...
endmodule""" % int(math.log(rom_size, 2)))
